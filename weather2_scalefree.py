...
#    nn = 49
    ...
    f = open('weather2_scalefree_output', 'w')
...
#    data_VA_KY, mu_VA_KY, stdev_VA_KY = scale(data_VA_KY)
    ...
    print("Building RC...")
    rc = scalefreeRC(5 * num_samples_VA, nn, 5 * num_samples_VA, sparsity=sparsity)
...
#    rc.visual_train(U_train, y_train, gamma=gamma, filename="weather_viz.mp4")
    ...
    np.savetxt('Wout2_sf_VA', rc.Wout)
    print("Saving the reservoir weights for VA...")
    np.savetxt('W2_sf_VA', rc.Wres)
    # unscale the data
    y_units = unscale(y_test[:,-5:], mu_VA, stdev_VA)
    preds_units = unscale(preds[:,-5:], mu_VA, stdev_VA)
    np.savetxt('va_sf_t2', t)
    np.savetxt('va_sf_test2', y_units)
    np.savetxt('va_sf_preds2', preds_units)
    ...
    rc2 = scalefreeRC(5 * num_samples_UT, nn, 5 * num_samples_UT, sparsity=sparsity)
    ...
    np.savetxt('ut_sf_t2', t)
    np.savetxt('ut_sf_test2', y_units)
    np.savetxt('ut_sf_preds2', preds_units)
    f.close()
